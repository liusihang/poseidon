...
// =================== 角色选择（主/副板）===================
#define BOARD_ROLE_PRIMARY 0  // 1=主板(P1/P2)；0=副板(P3/P4)

// =================== 引脚映射（UNO + CNC Shield v3）======
#define EN_PIN   8   // TMC2209 ENN：低电平=使能
#define X_STP    2
#define X_DIR    5
#define Y_STP    3
#define Y_DIR    6
...
// =================== 运动默认参数（保守稳健）=============
// 注意：1/32 微步下，同样的“步/秒”对应的“角速度/线速度”会更低；请根据校准换算。
#define DEFAULT_VMAX   500.0f   // steps/s
#define DEFAULT_ACCEL  500.0f   // steps/s^2

// =================== TMC2209 STEP 时序 ====================
// TMC2209 对 STEP 脉宽/方向建立保持有要求；3us 脉宽保守安全。
#define MIN_PULSE_US   3        // STEP 高电平最小脉宽
// 如方向易抖，可在改向前加微小延时（此处无需，AccelStepper 内部已处理）
...
AccelStepper* axisOfPump(int pump){
  ...
  return nullptr;
#else
  if (pump==3) return &stepperY;
  if (pump==4) return &stepperX;
  return nullptr;
#endif
}
...
void setup(){
  ...
  enableDrivers(false); 
                // 上电默认失能（安全）
  #if BOARD_ROLE_PRIMARY
    Serial.println(F("ROLE=PRIMARY (P1/P2 active)"));
  #else
    Serial.println(F("ROLE=SECONDARY (P3/P4 active)"));
  #endif
  for (int i=0;i<3;++i){
  ...
  Serial.println(F("FW READY (CSV,230400,TMC2209-STEP/DIR)"));
